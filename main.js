...
for (const pd of productList.list){
    if (pd.tags.includes("전자제품"))
        products.push(new ElectronicProduct(pd.name, pd.description, pd.price, pd.tags, pd.images));
    else
        products.push(new Product(pd.name, pd.description, pd.price, pd.tags, pd.images, undefined, pd.manufacturer));
}
